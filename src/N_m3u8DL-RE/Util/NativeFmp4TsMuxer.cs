...
internal sealed class NativeFmp4TsMuxer
{
    ...
    public static async Task<bool> RunAsync(string[] pipeNames, string outputPath)
    {
        ...
        try
        {
            ...
            await using var dst = new FileStream(outputPath, FileMode.Open, FileAccess.Write, FileShare.ReadWrite, 1024 * 1024, FileOptions.Asynchronous | FileOptions.SequentialScan);
    ...
    private Track ParseInit(byte[] moov, int pipeIndex)
    {
        foreach (var trak in FindBoxes(moov, "trak"))
        {
            ...
            var hp = h.Value.Payload; var handler = Encoding.ASCII.GetString(hp, 8, 4);
            var t = new Track { Kind = handler == "vide" ? Kind.Video : handler == "soun" ? Kind.Audio : Kind.Unknown }; if (t.Kind == Kind.Unknown) continue;
            Logger.WarnMarkUp($"[PIPE-DIAG] reader={pipeIndex} track handler={handler}");
            var tkhd = FindBox(trak.Payload, "tkhd");
            if (tkhd != null) { var p = tkhd.Value.Payload; if (p.Length >= 20 && p[0] == 0) t.TrackId = ReadU32(p, 12); else if (p.Length >= 32 && p[0] == 1) t.TrackId = ReadU32(p, 20); }
            var mdhd = FindBox(trak.Payload, "mdhd");
            if (mdhd != null) { var p = mdhd.Value.Payload; if (p.Length >= 24 && p[0] == 1) t.TimeScale = BinaryPrimitives.ReadUInt32BigEndian(p.AsSpan(20, 4)); else if (p.Length >= 16) t.TimeScale = BinaryPrimitives.ReadUInt32BigEndian(p.AsSpan(12, 4)); }
            if (t.TimeScale == 0) t.TimeScale = t.Kind == Kind.Audio ? 48000u : 90000u;
            var stsd = FindBox(trak.Payload, "stsd"); if (stsd == null) throw new InvalidDataException("Track has no stsd");
            var entry = FindSampleEntry(stsd.Value.Payload, t.Kind == Kind.Video); if (entry == null) throw new InvalidDataException($"No supported {(t.Kind == Kind.Video ? "video" : "audio")} sample entry");
            Logger.WarnMarkUp($"[PIPE-DIAG] reader={pipeIndex} sample-entry={entry.Value.Type} bytes={entry.Value.Payload.Length}");
            if (t.Kind == Kind.Video)
            {
                var avc = FindBoxInSampleEntry(entry.Value.Payload, entry.Value.Type, "avcC"); var hvc = FindBoxInSampleEntry(entry.Value.Payload, entry.Value.Type, "hvcC");
            ...
            {
                var esds = FindBoxInSampleEntry(entry.Value.Payload, entry.Value.Type, "esds"); var asc = esds == null ? null : FindDescriptor(esds.Value.Payload, 0x05);
            ...
            {
                var p = trex.Payload; if (p.Length < 24) continue; var trackId = ReadU32(p, 4); if (trackId != t.TrackId) continue;
                t.DefaultSampleDescriptionIndex = ReadU32(p, 8); t.TrexDefaultDuration = ReadU32(p, 12); t.TrexDefaultSize = ReadU32(p, 16); t.TrexDefaultFlags = ReadU32(p, 20);
                Logger.WarnMarkUp($"[PIPE-DIAG] reader={pipeIndex} trex track={trackId} defDur={t.TrexDefaultDuration} defSize={t.TrexDefaultSize} defFlags=0x{t.TrexDefaultFlags:X8}"); break;
            }
            return t;
        }
    ...
    {
        if (stsd.Length < 8) return null; var count = BinaryPrimitives.ReadUInt32BigEndian(stsd.AsSpan(4, 4)); var pos = 8;
        for (uint i = 0; i < count && pos + 8 <= stsd.Length; i++) { var size = BinaryPrimitives.ReadUInt32BigEndian(stsd.AsSpan(pos, 4)); if (size < 8 || size > stsd.Length - pos) return null; var type = Encoding.ASCII.GetString(stsd, pos + 4, 4); var payload = stsd.AsSpan(pos + 8, checked((int)size - 8)).ToArray(); if ((video && IsVideoSampleEntry(type)) || (!video && IsAudioSampleEntry(type))) return new Box(type, payload); pos += checked((int)size); }
        return null;
    }

    private static Box? FindBoxInSampleEntry(byte[] payload, string entryType, string target)
    {
        var fixedHeader = IsVideoSampleEntry(entryType) ? 78 : IsAudioSampleEntry(entryType) ? 28 : 0; if (payload.Length < fixedHeader) return null;
        var children = payload.AsSpan(fixedHeader).ToArray(); var found = FindBox(children, target); if (found != null) return found;
        foreach (var child in Boxes(children)) if (IsVideoSampleEntry(child.Type) || IsAudioSampleEntry(child.Type)) { found = FindBoxInSampleEntry(child.Payload, child.Type, target); if (found != null) return found; }
        return null;
    }
    ...
    private static List<Sample> ParseFragment(byte[] moof, byte[] mdat, Track t)
    {
        ...
        {
            var tfhd = FindBox(traf.Payload, "tfhd"); var tfdt = FindBox(traf.Payload, "tfdt"); var trun = FindBox(traf.Payload, "trun"); if (trun == null) continue;
            var tfhdPayload = tfhd?.Payload; var trunPayload = trun.Value.Payload;
            if (trunPayload.Length < 8) { Logger.WarnMarkUp($"[PIPE-DIAG] fragment {t.Kind}: malformed trun payload={trunPayload.Length}"); continue; }
            var tfhdFlags = tfhdPayload != null && tfhdPayload.Length >= 8 ? ReadU24(tfhdPayload, 0) : 0; var trunFlags = ReadU24(trunPayload, 0); var sampleCount = ReadU32(trunPayload, 4);
            ...
            ulong dts = 0;
            if (tfdt != null && tfdt.Value.Payload.Length >= 8) { var p = tfdt.Value.Payload; if (p[0] == 1 && p.Length >= 12) dts = BinaryPrimitives.ReadUInt64BigEndian(p.AsSpan(4, 8)); else dts = BinaryPrimitives.ReadUInt32BigEndian(p.AsSpan(4, 4)); }
            uint defDur = 0, defSize = 0, defFlags = 0; long baseDataOffset = -1;
            if (tfhdPayload != null && tfhdPayload.Length >= 8)
            {
                var pos = 8;
                if ((tfhdFlags & 0x000001) != 0) { if (pos + 8 > tfhdPayload.Length) continue; baseDataOffset = unchecked((long)BinaryPrimitives.ReadUInt64BigEndian(tfhdPayload.AsSpan(pos, 8))); pos += 8; }
                if ((tfhdFlags & 0x000002) != 0) { if (pos + 4 > tfhdPayload.Length) continue; pos += 4; }
                if ((tfhdFlags & 0x000008) != 0) { if (pos + 4 > tfhdPayload.Length) continue; defDur = ReadU32(tfhdPayload, pos); pos += 4; }
                if ((tfhdFlags & 0x000010) != 0) { if (pos + 4 > tfhdPayload.Length) continue; defSize = ReadU32(tfhdPayload, pos); pos += 4; }
                if ((tfhdFlags & 0x000020) != 0) { if (pos + 4 > tfhdPayload.Length) continue; defFlags = ReadU32(tfhdPayload, pos); }
            }
            ...
            var pos2 = 8; long dataPos;
            if ((trunFlags & 0x000001) != 0)
            {
                if (pos2 + 4 > trunPayload.Length) continue; var dataOffset = unchecked((int)ReadU32(trunPayload, pos2)); pos2 += 4; var moofBoxSize = 8L + moof.Length;
                dataPos = (tfhdFlags & 0x020000) != 0 ? dataOffset - moofBoxSize : baseDataOffset >= 0 ? baseDataOffset + dataOffset - moofBoxSize : dataOffset - moofBoxSize;
                Logger.WarnMarkUp($"[PIPE-DIAG] fragment {t.Kind}: dataOffset={dataOffset} moofBoxSize={moofBoxSize} resolvedDataPos={dataPos}");
            }
            else dataPos = (tfhdFlags & 0x020000) != 0 || baseDataOffset < 0 ? 0 : baseDataOffset - (8L + moof.Length);
            ...
            {
                var dur = defDur; var size = defSize; var sf = defFlags; var cto = 0;
                if (compactDurationSize)
                {
                    if (pos2 + 8 > sampleRecordEnd) { Logger.WarnMarkUp($"[PIPE-DIAG] fragment {t.Kind}: sample {i} missing compatibility duration/size"); break; }
                    dur = ReadU32(trunPayload, pos2); pos2 += 4; size = ReadU32(trunPayload, pos2); pos2 += 4;
                }
                else if (compatibilityCtoBytes)
                {
                    if (pos2 + 12 > sampleRecordEnd) { Logger.WarnMarkUp($"[PIPE-DIAG] fragment {t.Kind}: sample {i} missing compatibility duration/size/CTO"); break; }
                    dur = ReadU32(trunPayload, pos2); pos2 += 4; size = ReadU32(trunPayload, pos2); pos2 += 4; cto = unchecked((int)ReadU32(trunPayload, pos2)); pos2 += 4;
                }
                else
                {
                    if ((trunFlags & 0x000100) != 0) { if (pos2 + 4 > trunPayload.Length) break; dur = ReadU32(trunPayload, pos2); pos2 += 4; }
                    if ((trunFlags & 0x000200) != 0) { if (pos2 + 4 > trunPayload.Length) break; size = ReadU32(trunPayload, pos2); pos2 += 4; }
                    if ((trunFlags & 0x000400) != 0) { if (pos2 + 4 > trunPayload.Length) break; sf = ReadU32(trunPayload, pos2); pos2 += 4; }
                    if ((trunFlags & 0x000800) != 0) { if (pos2 + 4 > trunPayload.Length) break; cto = unchecked((int)ReadU32(trunPayload, pos2)); pos2 += 4; }
                }
                ...
                if (i < 2) Logger.WarnMarkUp($"[PIPE-DIAG] fragment {t.Kind}: sample {i} duration={dur} size={size} cto={cto} dataPos={dataPos}");
                result.Add(new Sample(mdat.AsSpan(checked((int)dataPos), checked((int)size)).ToArray(), dts, cto, (sf & 0x10000) == 0)); dts += dur; dataPos += size;
            }
        }
        return result;
    }
